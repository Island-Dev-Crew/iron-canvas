...
```
> ★v6 note: when Lenis drives the scroll, leave `scroll-behavior` at `auto` (the two fight), and
> gate any native smooth scroll behind `@media (prefers-reduced-motion: no-preference)`.
